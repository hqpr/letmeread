...
def main():
    ...
    c = urllib.urlopen('http://www.livelib.ru/books/movers-and-shakers/~4') # открываем 50 самых популярных
    ...
    for t in text:
        ...
        for b in book:
            ...
            sql = "INSERT INTO page_book (title, smallimage, isbn, year, publisher, source) VALUES ('%s','%s','%s','%s','%s','%s')" % (name, smallimg, isbn, year, publisher, fullstoryUrl)
            try:
                cur.execute(sql)
                db.commit()
                print 'updated'
...
#        print 'Book picture with id # %s successfully downloaded ' % id
        ...
        sql = "UPDATE page_book SET name = '%s', author = '%s', image = '%s', text ='%s', rating = '%s' WHERE id = '%s'" % (bookname, author, local_pic, story, rating, id)
